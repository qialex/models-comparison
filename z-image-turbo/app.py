"""Minimal Z-Image-Turbo text-to-image API (Tongyi-MAI).

Same shape as sdxl-turbo/flux: GET /health, POST /generate, POST /generate.png.
Card: guidance_scale=0, num_inference_steps=9 (~8 DiT NFEs), prefer 1024².
6B — on 12GB use DEVICE_MODE=offload|sequential; full cuda wants ~16GB.
"""
...
def load_pipeline():
    ...
    # Card recommends bfloat16; fall back to float16 if unsupported.
    ...
    if torch.cuda.is_available():
        if DEVICE_MODE == "sequential":
            ...
            # Default for 12GB: model CPU offload (card documents this path).
            try:
...
async def lifespan(_app: FastAPI):
    ...
    if not torch.cuda.is_available():
        raise RuntimeError("CUDA required for Z-Image-Turbo")
    pipe = load_pipeline()
    ready = True
    yield
    ready = False
...
app = FastAPI(title="z-image-turbo", lifespan=lifespan)


class GenerateRequest(BaseModel):
    prompt: str = Field(..., min_length=1)
    negative_prompt: str | None = None  # unused when guidance_scale=0
