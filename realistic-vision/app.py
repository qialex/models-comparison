"""Minimal Realistic Vision V5.1 text-to-image API (SD 1.5).

Same shape as juggernaut/flux: GET /health, POST /generate, POST /generate.png.
Loads recommended MSE VAE. Safety checker disabled for adult bench use.
"""
...
# Model-card recommended negative (trimmed default for photoreal).
...
def load_pipeline():
    ...
    try:
        p = StableDiffusionPipeline.from_pretrained(MODEL_ID, **kwargs)
    except TypeError:
        kwargs.pop("requires_safety_checker", None)
        p = StableDiffusionPipeline.from_pretrained(MODEL_ID, **kwargs)

    # DPM++ 2M Karras (model card recommendation alongside Euler A)
...
async def lifespan(_app: FastAPI):
    ...
    if not torch.cuda.is_available():
        raise RuntimeError("CUDA required for Realistic Vision V5.1")
    pipe = load_pipeline()
    ready = True
    yield
    ready = False
...
app = FastAPI(title="realistic-vision-v5.1", lifespan=lifespan)
