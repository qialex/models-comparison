"""Minimal Juggernaut XI v11 text-to-image API (SDXL).

Same shape as juggernaut-xl: GET /health, POST /generate, POST /generate.png.
Card: 832x1216, DPM++ 2M Karras, 30-40 steps, CFG 3-7. Gated HF repo — set HF_TOKEN.
...
MODEL_ID = os.environ.get("MODEL_ID", "RunDiffusion/Juggernaut-XI-v11")
...
async def lifespan(_app: FastAPI):
    ...
    if not torch.cuda.is_available():
        raise RuntimeError("CUDA required for Juggernaut-XI-v11")
    pipe = load_pipeline()
    ready = True
    yield
    ready = False
...
app = FastAPI(title="juggernaut-xi-v11", lifespan=lifespan)
