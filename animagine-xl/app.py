"""Minimal Animagine XL 4.0 text-to-image API (anime SDXL).

Same shape as juggernaut/flux: GET /health, POST /generate, POST /generate.png.
Defaults from model card: 832x1216, 28 steps, CFG 5, Euler a.
"""
...
MODEL_ID = os.environ.get("MODEL_ID", "cagliostrolab/animagine-xl-4.0")
...
USE_LPW = os.environ.get("USE_LPW", "1").strip().lower() in ("1", "true", "yes")
...
def load_pipeline():
    ...
    try:
        p = StableDiffusionXLPipeline.from_pretrained(MODEL_ID, **kwargs)
    except Exception:
        kwargs.pop("custom_pipeline", None)
        p = StableDiffusionXLPipeline.from_pretrained(MODEL_ID, **kwargs)

    # Euler a (model card preferred sampler)
...
async def lifespan(_app: FastAPI):
    ...
    if not torch.cuda.is_available():
        raise RuntimeError("CUDA required for Animagine XL 4.0")
    pipe = load_pipeline()
    ready = True
    yield
    ready = False
...
app = FastAPI(title="animagine-xl-4.0", lifespan=lifespan)
...
def generate(body: GenerateRequest):
    ...
    neg = body.negative_prompt if body.negative_prompt is not None else DEFAULT_NEGATIVE
