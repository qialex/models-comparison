#!/usr/bin/env python3
"""10 repeated searches: cheapest Flux core (effVRAM + RAM only). No rent."""
...
OUT = cache_debug("flux2-klein-9b-kv-int8", "_debug") / "search10_core_cheapest.json"
MIN_EFF = 11.5
MIN_RAM = 30
N = 10
...
def search():
    return json.loads(
        subprocess.check_output(
            ["vastai", "search", "offers", QUERY, "--order", "dph_total", "--limit", "80", "--raw", "-n"],
            text=True,
        )
    )
...
def fit(rows):
    return sorted(
        [r for r in rows if r["eff_vram"] >= MIN_EFF and r["ram_g"] > MIN_RAM],
...
def main():
    ...
    for i in range(1, N + 1):
        ...
        hits = fit(rows)
        top = hits[:8]
        print(f"\n=== search {i}/{N}  pool={len(rows)}  core_fit={len(hits)} ===")
        for r in top:
            print(
    ...
    summary = {
        "query": QUERY,
        "filters": "eff_vram>=11.5 AND ram_g>30 only (no static/ports/reli/verified)",
        "n_searches": N,
