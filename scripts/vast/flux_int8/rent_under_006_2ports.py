#!/usr/bin/env python3
"""Find/rent Flux-fit offer under MAX_DPH with direct_port_count>=2."""
...
def normalize(o):
    ...
    return price, vram_g, ram_g, ports, o


def candidates(offers):
    rows = []
    for o in offers:
        price, vram_g, ram_g, ports, o = normalize(o)
        if price >= MAX_DPH:
            continue
        ...
        if ports < 2:
            continue
        rows.append((price, vram_g, ram_g, ports, o))
...
def main() -> int:
    ...
    for attempt in range(1, MAX_ATTEMPTS + 1):
        ...
        cheap = [r for r in rows if r[4]["id"] not in tried]
        print(f"\n[{attempt}/{MAX_ATTEMPTS}] <${MAX_DPH} + ports>=2: {len(rows)} fit, {len(cheap)} untried")
        for price, vram_g, ram_g, ports, o in rows[:10]:
            mark = "tried" if o["id"] in tried else "new"
            print(
                f"  {o['id']} ${price:.4f} {o.get('gpu_name')} "
                f"vram={vram_g:.1f}G ram={ram_g:.0f}G ports={ports} "
                f"{o.get('geolocation')} [{mark}]"
            )
        if not cheap:
        ...
        price, vram_g, ram_g, ports, o = cheap[0]
        oid = o["id"]
        tried.add(oid)
        print(f"renting {oid} @ ${price:.4f} ports={ports} ...")
        res = create(oid)
        print(
            json.dumps(
                {
        ...
        if res.get("success") and res.get("new_contract"):
            print(
                f"SUCCESS instance={res['new_contract']} offer={oid} "
                f"${price:.4f} ports={ports}"
            )
            return 0
        time.sleep(1)
